...
wgrd_files::File::File(FileMeta meta, fs::path out_path) : out_path(out_path) {
  ...
  xml_path =
      out_path / "xml" / fs::path(meta.vfs_path).replace_extension(".xml");
}
...
void wgrd_files::Files::add_file(fs::path out_path, FileMeta meta,
                                 size_t file_offset) {
  ...
  } else if (EDat::is_file(meta.vfs_path, f, meta.offset)) {
    files[meta.idx] = std::make_unique<EDat>(meta, out_path);
  } else if (Ess::is_file(meta.vfs_path, f, meta.offset)) {
    files[meta.idx] = std::make_unique<Ess>(meta, out_path);
  } else if (SFormat::is_file(meta.vfs_path, f, meta.offset)) {
    files[meta.idx] = std::make_unique<SFormat>(meta, out_path);
  } else if (TGV::is_file(meta.vfs_path, f, meta.offset)) {
    files[meta.idx] = std::make_unique<TGV>(meta, out_path);
  } else if (PPK::is_file(meta.vfs_path, f, meta.offset)) {
    files[meta.idx] = std::make_unique<PPK>(meta, out_path);
  } else if (Scenario::is_file(meta.vfs_path, f, meta.offset)) {
    files[meta.idx] = std::make_unique<Scenario>(meta, out_path);
  } else if (NdfBin::is_file(meta.vfs_path, f, meta.offset)) {
...
void wgrd_files::Files::copy_bin_changes(fs::path dat_path,
  ...
  for (auto &[idx, file] : files) {
    if (file->is_changed()) {
      if (file->fs_path == dat_path) {
        ...
        spdlog::info("Saving binary for {} to {}", file->vfs_path,
                     bin_path.string());
